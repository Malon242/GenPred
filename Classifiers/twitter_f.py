...
def preprocess(df):
	...
	df['label'] = df['gender'].apply(lambda x: 'F' if x == 'F' else 'NOT F')
...
def tagger(txt, nlp, tags, abbrev_lst, punct_lst, prof_lst):
	"""Change some tokens to their corresponding pos tag, except for puctuation,
...
def main():
	...
	tw_results.to_csv("../Results/tw_f.csv", index=False)
	...
	r_results.to_csv("../Results/r_tw_f.csv", index=False)
